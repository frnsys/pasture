...
def similar(in_vecs, vecs, threshold=1.1, metric='cosine'):
    """
...
def most_similar(in_vecs, vecs, n=5, metric='cosine'):
    """
...
def dissimilar(in_vecs, vecs, threshold=1.1, metric='cosine'):
    sim_mat = cdist(in_vec.todense(), vecs.todense(), metric=metric)
    most_sim = np.where(np.any(sim_mat>threshold), axis=0)
    return most_sim

def least_similar(in_vecs, vecs, n=5, metric='cosine'):
    sim_mat = cdist(in_vecs.todense(), vecs.todense(), metric=metric)
    return np.fliplr(np.argsort(sim_mat).flatten())[:n]

def most_salient(vecs, n=5, metric='cosine'):
    """
    The most salient indices, that is, those that are most dissimilar from the bunch.
    """
    sim_mat = squareform(pdist(vecs.todense(), metric=metric))
    means = np.mean(sim_mat, axis=1)
    return np.argsort(means)[::-1][:n]

def most_representative(vecs, n=5, metric='cosine'):
    sim_mat = squareform(pdist(vecs.todense(), metric=metric))
    means = np.mean(sim_mat, axis=1)
